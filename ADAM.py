...
# Exibir o último índice utilizado
st.write(f"Último índice utilizado: {st.session_state.current_index}")
...
if st.button("Tokenizar"):
    try:
        ...
        if tokens:
            ...
            token_dict = {str(initial_index + i): f'"{token}"' for i, token in enumerate(tokens)}
            st.session_state.current_index = initial_index + len(tokens)  # Atualiza o índice para a próxima tokenização
